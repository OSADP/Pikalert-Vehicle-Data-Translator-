...
const std::string rwx_rtma_dataset::GEOPOTENTIAL_HEIGHT = "HGT_P0_L1_GLC0";
...
const float rwx_rtma_dataset::MISSING = -9999.;
...
int rwx_rtma_dataset::get_offset(double lat, double lon, size_t *offset)
{
  double xc;
  double yc;
...
int rwx_rtma_dataset::get_coord(double lat, double lon, double *xc, double *yc)
{
  double xcoord;
  double ycoord;
  
  rtma_proj->ll2xy(lon, lat, &xcoord, &ycoord);
...
int rwx_rtma_dataset::get_coord_offset(double lat, double lon, double *xc, double *yc, size_t *offset)
{
  double xcoord;
  double ycoord;
...
void rwx_rtma_dataset::get_temperature_celsius(size_t offset, float &value)
{
  float temperature = (*temperature_field)[offset];
  if (temperature == temperature_missing)
    {
...
void rwx_rtma_dataset::get_dewpoint_celsius(size_t offset, float &value)
{
  float dewpoint = (*dewpoint_field)[offset];
  if (dewpoint == dewpoint_missing)
    {
...
void rwx_rtma_dataset::get_pressure_mb(size_t offset, float &value)
{
  float pressure = (*pressure_field)[offset];
  if (pressure == pressure_missing)
    {
...
void rwx_rtma_dataset::get_wind_dir_deg(size_t offset, float &value)
{
...
void rwx_rtma_dataset::get_wind_speed_ms(size_t offset, float &value)
{
...
void rwx_rtma_dataset::get_geopotential_height(size_t offset, float &value)
{
