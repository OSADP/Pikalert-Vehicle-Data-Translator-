...
{
  road_segment_id = seg_stat.get_id();
...
int vdt_road_segment_assessment::visibility_assessment(vdt_probe_message_qc_statistics& seg_stat, int precip_assessment)
{
  ...
  if (precip_assessment == PRECIP_FROZEN ||
    ...
    {
      if (seg_stat.get_nss_wind_speed_mean() == vdt_const::FILL_VALUE)
        {
          return vdt_const::FILL_VALUE;
        }
      else if (seg_stat.get_nss_wind_speed_mean() > 10)
        {
...
int vdt_road_segment_assessment::visibility_assessment(vdt_probe_message_qc_statistics& seg_stat)
{
  ...
  if (seg_stat.get_nss_prevail_vis_mean() != vdt_const::FILL_VALUE &&
      seg_stat.get_nss_prevail_vis_mean() <= 5)
    {
      interest += 0.25;
    }
  if (interest >= 0.5)
...
int vdt_road_segment_assessment::get_wipers_on(vdt_probe_message_qc_statistics& seg_stat)
{
  ...
  if (seg_stat.get_num_wipers_intermittent() != vdt_const::FILL_VALUE)
  {
    num_wipers_on += seg_stat.get_num_wipers_intermittent();
  }

  if (seg_stat.get_num_wipers_low() != vdt_const::FILL_VALUE)
  {
    num_wipers_on += seg_stat.get_num_wipers_low();
  }
 
  if (seg_stat.get_num_wipers_high() != vdt_const::FILL_VALUE)
  {
    num_wipers_on += seg_stat.get_num_wipers_high();
  }

  if (seg_stat.get_num_wipers_automatic() != vdt_const::FILL_VALUE)
  {
    num_wipers_on += seg_stat.get_num_wipers_automatic();
  }
...
{
  if (seg_stat.get_air_temp_mean() == vdt_const::FILL_VALUE &&
      seg_stat.get_air_temp2_mean() == vdt_const::FILL_VALUE)
    {
      return vdt_const::FILL_VALUE;
    }
  //use air_temp_mean if it is there, if not fall back on air_temp2_mean
  float air_temp = seg_stat.get_air_temp_mean() != vdt_const::FILL_VALUE ? seg_stat.get_air_temp_mean() : seg_stat.get_air_temp2_mean();
  return air_temp;
}

bool vdt_road_segment_assessment::wipers_intermittent(vdt_probe_message_qc_statistics& seg_stat)
{
  return (seg_stat.get_num_wipers_intermittent() > seg_stat.get_num_wipers_low() &&
          seg_stat.get_num_wipers_intermittent() > seg_stat.get_num_wipers_high() &&
          seg_stat.get_num_wipers_intermittent() > seg_stat.get_num_wipers_off());
}

bool vdt_road_segment_assessment::wipers_high(vdt_probe_message_qc_statistics& seg_stat)
{
  return (seg_stat.get_num_wipers_high() > seg_stat.get_num_wipers_intermittent() &&
          seg_stat.get_num_wipers_high() > seg_stat.get_num_wipers_off() &&
          seg_stat.get_num_wipers_high() > seg_stat.get_num_wipers_low());
}

bool vdt_road_segment_assessment::wipers_low(vdt_probe_message_qc_statistics& seg_stat)
{
  return (seg_stat.get_num_wipers_low() > seg_stat.get_num_wipers_intermittent() &&
          seg_stat.get_num_wipers_low() > seg_stat.get_num_wipers_high() &&
          seg_stat.get_num_wipers_low() > seg_stat.get_num_wipers_off());
}

bool vdt_road_segment_assessment::wipers_off(vdt_probe_message_qc_statistics& seg_stat)
{
  return (seg_stat.get_num_wipers_off() > seg_stat.get_num_wipers_intermittent() &&
          seg_stat.get_num_wipers_off() > seg_stat.get_num_wipers_high() &&
          seg_stat.get_num_wipers_off() > seg_stat.get_num_wipers_low());
}
...
{
  return (seg_stat.get_num_msg_valid_abs() > 0 &&
          seg_stat.get_num_abs_engaged() > (seg_stat.get_num_abs_on() + seg_stat.get_num_abs_off()));
}

bool vdt_road_segment_assessment::abs_not_engaged(vdt_probe_message_qc_statistics& seg_stat)
{
  return (seg_stat.get_num_msg_valid_abs() > 0 &&
          (seg_stat.get_num_abs_on() + seg_stat.get_num_abs_off()) > seg_stat.get_num_abs_engaged());
}

bool vdt_road_segment_assessment::trac_engaged(vdt_probe_message_qc_statistics& seg_stat)
{
  return (seg_stat.get_num_msg_valid_trac() > 0 &&
          seg_stat.get_num_trac_engaged() > (seg_stat.get_num_trac_on() + seg_stat.get_num_trac_off()));
}

bool vdt_road_segment_assessment::trac_not_engaged(vdt_probe_message_qc_statistics& seg_stat)
{
  return (seg_stat.get_num_msg_valid_trac() > 0 &&
          (seg_stat.get_num_trac_on() + seg_stat.get_num_trac_off()) > seg_stat.get_num_trac_engaged());
}

bool vdt_road_segment_assessment::stab_engaged(vdt_probe_message_qc_statistics& seg_stat)
{
  return (seg_stat.get_num_msg_valid_stab() > 0 &&
          seg_stat.get_num_stab_engaged() > (seg_stat.get_num_stab_on() + seg_stat.get_num_stab_off()));
}

bool vdt_road_segment_assessment::stab_not_engaged(vdt_probe_message_qc_statistics& seg_stat)
{
  return (seg_stat.get_num_msg_valid_stab() > 0 &&
          (seg_stat.get_num_stab_on() + seg_stat.get_num_stab_off()) > seg_stat.get_num_stab_engaged());
}

bool vdt_road_segment_assessment::wipers_missing(vdt_probe_message_qc_statistics& seg_stat)
{
  return (seg_stat.get_num_wipers_off() == vdt_const::FILL_VALUE &&
          seg_stat.get_num_wipers_intermittent() == vdt_const::FILL_VALUE &&
          seg_stat.get_num_wipers_low() == vdt_const::FILL_VALUE &&
          seg_stat.get_num_wipers_high() == vdt_const::FILL_VALUE);
}

int vdt_road_segment_assessment::precip_type_frozen_branch(vdt_probe_message_qc_statistics& seg_stat)
{
  float radar_ref = seg_stat.get_radar_ref();
  if (radar_ref == vdt_const::FILL_VALUE)
    {
      return vdt_const::FILL_VALUE;
    }
  float speed_ratio = seg_stat.get_speed_ratio();
  if (radar_ref < -10)
    {
      return NO_PRECIP;
    }
...
int vdt_road_segment_assessment::precip_type_mixed_branch(vdt_probe_message_qc_statistics& seg_stat)
{
  ...
  int cloud_mask = seg_stat.get_cloud_mask();
  float speed_ratio = seg_stat.get_speed_ratio();
  float air_temp = get_air_temp(seg_stat);
  float vis = seg_stat.get_nss_prevail_vis_mean();
  if (radar_ref <= -99)
    {
      if (wipers_missing(seg_stat))
        {
...
int vdt_road_segment_assessment::precip_type_rain_branch(vdt_probe_message_qc_statistics& seg_stat)
{
  ...
  if (radar_ref >= 0 &&
      radar_ref <= 30)
    {
      ...
      else if (seg_stat.get_num_wipers_intermittent() > 0 ||
               seg_stat.get_num_wipers_low() > 0)
        {
          return PRECIP_RAIN;
        }
      else if (seg_stat.get_num_wipers_high() > 0)
        {
          return PRECIP_HEAVY_RAIN;
        }
    }
  if (radar_ref > 30)
    {
      if (seg_stat.get_num_wipers_intermittent() > 0 ||
          seg_stat.get_num_wipers_off() > 0)
        {
          return PRECIP_RAIN;
        }
      else if (seg_stat.get_num_wipers_low() > 0 ||
               seg_stat.get_num_wipers_high() > 0)
        {
          return PRECIP_HEAVY_RAIN;
        }
    }
...
{
  double interest = 0.0;
  if (seg_stat.get_speed_ratio() != vdt_const::FILL_VALUE)
    {
      interest += seg_stat.get_speed_ratio();
    }
  float air_temp = get_air_temp(seg_stat);
  if (air_temp != vdt_const::FILL_VALUE)
    {
  ...
  interest += get_interest(seg_stat.get_radar_ref(),0,15,-0.033,0.995);
  interest += get_interest(seg_stat.get_radar_ref(),15,22,-0.071,1.565);
  ...
  interest += get_interest(seg_stat.get_lat_accel_mean(),0,0.2,1);
  interest += get_interest(seg_stat.get_lat_accel_mean(),0.2,0.24,-25,6);
  interest += get_interest(seg_stat.get_lon_accel_mean(),0,0.5,1);
  interest += get_interest(seg_stat.get_lon_accel_mean(),0.5,1,-2,2);
  interest += get_interest(seg_stat.get_yaw_mean(),0,1,1);
  interest += get_interest(seg_stat.get_yaw_mean(),1,1.7,-1.428,2.428);
  interest += get_interest(seg_stat.get_steering_angle_mean(),0,4,-0.25,1);
  return interest;
}
...
double vdt_road_segment_assessment::get_wet_pavement(vdt_probe_message_qc_statistics& seg_stat)
{
  ...
  interest += get_interest(seg_stat.get_speed_ratio(), 0, 0.8, 1.25, 0);
  interest += get_interest(seg_stat.get_speed_ratio(), 0.8, 1.0, -1.5, 2.2);

  float air_temp = get_air_temp(seg_stat);
  interest += get_interest(air_temp, -10, 0, 0.1, 1);
  interest += get_interest(air_temp, 0, 5, 1);

  interest += get_interest(seg_stat.get_radar_ref(), 0, 10, 0.5);
  interest += get_interest(seg_stat.get_radar_ref(), 10, 20, 0.05, 0);
  ...
  interest += get_interest(seg_stat.get_lat_accel_mean(), 0, 0.2, 1);
  interest += get_interest(seg_stat.get_lat_accel_mean(), 0.2, 0.3, -10, 3);
  interest += get_interest(seg_stat.get_lon_accel_mean(), 0, 0.5, 1);
  interest += get_interest(seg_stat.get_lon_accel_mean(), 0.5, 1, -2, 2);
  interest += get_interest(seg_stat.get_yaw_mean(), 0, 1, 1);
  interest += get_interest(seg_stat.get_yaw_mean(), 1, 1.7, -1.428, 2.428);
...
{
  double interest = 0.0;
  interest += get_interest(seg_stat.get_speed_ratio(), 0, 0.6, 0.833, 0.502);
  interest += get_interest(seg_stat.get_speed_ratio(), 0.6, 1, -2, 2.2);

  interest += get_interest(seg_stat.get_air_temp_mean(), -10, 0, 1);
  interest += get_interest(seg_stat.get_air_temp_mean(), 0, 1, -1, 1);

  interest += get_interest(seg_stat.get_radar_ref(),0, 20, 0.025, 0.5);
  interest += get_interest(seg_stat.get_radar_ref(),20, 25, 1);
  if (wipers_on(seg_stat))
    {
      interest += 1;
    }
  ...
  interest += get_interest(seg_stat.get_lat_accel_mean(),0,0.2,1);
  interest += get_interest(seg_stat.get_lat_accel_mean(),0.2,0.4,-5,2);
  interest += get_interest(seg_stat.get_lon_accel_mean(),0,0.5,1);
  interest += get_interest(seg_stat.get_lon_accel_mean(),0.5,1,-2,2);
  interest += get_interest(seg_stat.get_yaw_mean(),0,1,1);
  interest += get_interest(seg_stat.get_yaw_mean(),1,2,-1,2);
  interest += get_interest(seg_stat.get_steering_angle_mean(),0,4,-0.25,1);
  return interest;
}
...
{
  double interest = 0.0;
  interest += get_interest(seg_stat.get_speed_ratio(), 0, 0.8, -1.25, 1);

  interest += get_interest(seg_stat.get_air_temp_mean(), -10, -6, 1);
  interest += get_interest(seg_stat.get_air_temp_mean(), -6, 1, -0.1428, 0.1432);
  ...
  interest += get_interest(seg_stat.get_lat_accel_mean(), 0, 0.2, 5, 0);
  interest += get_interest(seg_stat.get_lat_accel_mean(), 0.2, 0.4, 1);
  interest += get_interest(seg_stat.get_lon_accel_mean(), 0.3, 0.5, 5, -1.5);
  interest += get_interest(seg_stat.get_lon_accel_mean(), 0.5, 1, 1);
  interest += get_interest(seg_stat.get_yaw_mean(), 0, 1, 1, 0);
  interest += get_interest(seg_stat.get_yaw_mean(), 1, 2, 1);
  interest += get_interest(seg_stat.get_steering_angle_mean(), 2, 3, 1, -2);
  interest += get_interest(seg_stat.get_steering_angle_mean(), 3, 4, 1);
  return interest;
}
...
double vdt_road_segment_assessment::get_road_splash_pavement(vdt_probe_message_qc_statistics& seg_stat)
{
  ...
  interest += get_interest(seg_stat.get_speed_ratio(),0,0.8,1.25,0);
  interest += get_interest(seg_stat.get_speed_ratio(),0.8,1,-1.5,2.2);

  interest += get_interest(seg_stat.get_air_temp_mean(),-10,0,0.1,1);
  interest += get_interest(seg_stat.get_air_temp_mean(),0,5,1);

  interest += get_interest(seg_stat.get_radar_ref(), 0, 25, 1);
