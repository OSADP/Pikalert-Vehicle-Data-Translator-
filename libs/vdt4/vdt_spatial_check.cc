...
int vdt_spatial_check_iqr::iqr_median(vector<float> &values, float *iqr25, float *iqr75, float *median)
{
  ...
  size_t num_values = values.size();
  size_t nofill_offset = 0;

  if(num_values > 0)
  {
    std::sort(values.begin(), values.end());
    auto it = std::upper_bound(values.begin(), values.end(), vdt_const::FILL_VALUE);
    size_t num_nofill = values.end() - it;
    nofill_offset = it - values.begin();
    assert(nofill_offset + num_nofill  == num_values && "Partition into 'fill' and 'nofill' was unsuccessful");
    num_values = num_nofill;
  }
  ...
  median_index = static_cast<int>(rint(num_values * 0.50)) + nofill_offset;
  low_quartile_index = static_cast<int>(rint(num_values * 0.25)) + nofill_offset;
  high_quartile_index = static_cast<int>(rint(num_values * 0.75)) + nofill_offset;

  if (num_values == 1)
    {
      median_index = nofill_offset;
      low_quartile_index = nofill_offset;
      high_quartile_index = nofill_offset;
    }
  else
    {
      median_index -= 1;
...
int vdt_spatial_check_iqr::perform_iqr(vector<float> &values, float target_reading) const
{
  ...
  if(iqr_median(values, &iqr25, &iqr75, &median) == -1)
  {
     return vdt_probe_message_qc::FAILED;
  }
