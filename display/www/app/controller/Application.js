...
Ext.define('emdss.controller.Application',{
    ...
    config:{
        refs:{
            ...
            mapNavView: 'main[id=map-nav-view]',
            mapView: 'main map',
    ...
    init: function() {
        ...
        // Road Segments
        // 
        this.roadSegments = Ext.create("emdss.store.Segments",{
            storeId:"segments-store"
        });
        this.roadSegments.setupAfterListener();
        var prefix = 'unk';
        if ( hrefState == 'colorado' )    { prefix = 'co'; }
        else if ( hrefState == 'michigan' )  { prefix = 'mi'; }
        else if ( hrefState == 'minnesota' ) { prefix = 'mn'; }
        else if ( hrefState == 'nevada' )    { prefix = 'nv'; }

        var appRoot = window.location.href.substr( 0, window.location.href.indexOf('?') );
        if ( appRoot.length == 0 ) {
            appRoot = window.location.href;
        }
        this.roadSegments.setProxy({
            type:'ajax',
            url:   appRoot
            ...
            reader: {
                    type:         'json',
                    rootProperty: 'segments'
                }
        });
        this.roadSegments.on({
            refresh:'onRoadSegmentsUpdate',
            scope:this
        });
        this.roadSegments.getProxy().addListener('exception', function (proxy, response, operation) {
                                                                  console.log(response.status);
                                                                  console.log(response.responseText);
                                                                  this.roadSegmentsAttemptedLoad = true;
                                                                  this.roadSegmentsLoadFailed = true;
                                                                  this.checkLoadmaskDisable();
                                                              }, this);
        this.roadSegments.load();
     ...
     */
    onRoadSegmentsUpdate: function(store, data, eOpts) {
        console.log("Updated road segments...");

        var items = data.items;
      
        console.log("    Loaded " + items.length + " segments...");
    ...
    /**
     * Handle update of new NWS watches and warnings from server -- called after the WxWatchWarns store is done loading.
     *   - For debug purposes
     */
    onWatchWarnsUpdate: function(store, data, eOpts) {
        
        console.log("Updated watches and warnings...");
    ...
    onTabChange: function(container, value, oldValue, eOpts) {
        var buttons = this.getBtnFcstType();
        var veh_button = this.getBtnVehicles();
        var rad_button = this.getBtnRadar();
        var green_segs_button = this.getBtnHideGreen();
        var toolbar = this.getMapToolbar();
        var radarPanel = this.getPanelRadar();
        var data_time_panel = this.getPanelMapTime();
