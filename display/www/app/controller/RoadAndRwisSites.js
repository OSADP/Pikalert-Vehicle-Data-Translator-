...
Ext.define('emdss.controller.RoadAndRwisSites', {
    ...
    addSitesToMap: function(records, map) {
        ...
        var segsStore = Ext.getStore('segments-store');
        if ( segsStore == null ) {
            console.log("Warning:");
            console.log("Warning: Segments store is null in addSitesToMap(). Would have added sites from " + records.length + " districts...");
            console.log("Warning:");
        ...
        for ( var key in records ) {
            ...
            sites.each(function (site, index, length) {
                ...
                var segment = segsStore.findRecord('dicast_id', site.get('site_num'));
                if ( segment ) {
                    var coords = segment.coords();
                    var coordArray = [];
                    coords.each(function (coord, index, length) {
                        coordArray.push( {'lat': coord.get('lat'), 'lng': coord.get('lon')} );
                    });
        ...
        if (me.segments) {
            for (var i = 0; i < me.segments.length; i++) {
                ...
                if (site_num == segment_id) {
                    ...
                    var segsStore = Ext.getStore('segments-store');
                    var segment = segsStore.findRecord('dicast_id', site_num);
                    if ( segment ) {
                        var coords = segment.coords();
                        var coordArray = [];
                        coords.each(function (coord, index, length) {
    ...
    pushShowSite: function(record, cmpId) {
        ...
        console.log("Road controller pushShowSite with site: " + record.get('site_id'));
        ...
        me.showSite.setSiteName(record.get('site_id'));
