...
Ext.define('emdss.util.Util', {
    ...
    getPrettyDateStringFromDateNoSecs: function (dateObj) {
        var me = this;
        var ap = 'am';
        var hrs = dateObj.getHours();
        if ( hrs >= 12 ) {
    ...
    /**
     * Converts a javascript Date object to an ISO-like string with no ms shown
     *   This is the format sent with the EMDSS travel time forecasts: 2016-05-10T21:45:00Z
     * 
     * @param {Object}
     *            date
     */
    dateToIsoStringNoMs : function(date) {
        var mon = date.getUTCMonth() + 1;
        var day = date.getUTCDate();
        var hour = date.getUTCHours();
        var min = date.getUTCMinutes();
