    ...
    setupAfterListener: function() {
        ...
        me.addAfterListener("load",
            function(records, operation, success, container){
                ...
                console.log("VEHICLES Loaded Data Time: " + dateString);

                var reader = me.getProxy().getReader();
                var directDataTime = reader.rawData.data_time;
                console.log("    Direct Data Time: " + directDataTime);

            });
    
        me.addListener("beforeload",
           function(records, operation, success, container){
               console.log("beforeLoad listener in Vehicle store removing all previous records...");
               me.removeAll(true);
           });
    },
            ...
            run: function() {
                ...
                if ( !me.isLoaded() && !me.isLoading() ) {
                    ...
                    //      emdss.util.Config.getDataTimesUrl());
                        ...
                        if ( !latestDataTimeStr || latestDataTimeStr.length < 1 ) {
                            console.log("");
                            console.log("");
                            console.log("");
                            console.log("");
                            console.log("");
                            console.log("");
                            console.log("NOTE: datatimes service returned empty string for latest vehicles time, meaning there are no vehicle data files on disk.");
                            console.log("    SKIPPING vehicle load...");
                            console.log("");
                            console.log("");
                            console.log("");
                            console.log("");
                        ...
                        if ( latestDate == undefined || loadedDate == undefined ) {
                            // TODO: Add counter here and reload after a few fails...
                            if ( typeof me.noDataCounter == 'undefined' || me.noDataCounter < 5 ) {
                                console.log("latestDate or loadedDate uninitialized for vehicle data. Sleeping...");
                                if ( typeof me.noDataCounter == 'undefined' ) {
                                    me.noDataCounter = 1;
                                }
                                else {
                                    me.noDataCounter++;
                                ...
                                console.log("latestDate or loadedDate uninitialized for vehicle data. Reloading because counter is: " + me.noDataCounter);
                                me.noDataCounter = 0;
                                me.load();
                            }
                        }
