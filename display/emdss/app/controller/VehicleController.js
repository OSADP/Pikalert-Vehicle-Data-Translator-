...
Ext.define('emdss.controller.VehicleController', {
     ...
     */
    onHideShowVehiclesTap: function(button, event, eOpts) {
     ...
     * Handle clicks to the vehicle visibility button.
     */
    // onHideShowVehiclesRelease: function(button, event, eOpts) {
    //     
    //     console.log("VehicleController::onHideShowVehiclesRelease()...");
    //     
    //     this.toggleShowVehicles(button);
    // },
        ...
        if ( senchaEvent.event.target.className === 'district_zoom' ) {
            ...
            districtVehicleStore.each(function (district, index, length) {
                ...
                if ( idx > -1 ) {
                    ...
                    // 
                    // Re-center on the vehicle
                    // 
                    var map = me.getMapView().getMap();
                    var newCenter = new google.maps.LatLng(vehicleRecord.get('lat_float'), vehicleRecord.get('lon_float'));
    ...
    onVehicleListUpdate: function(store, data, eOpts) {
        ...
        var vehiclesStore = Ext.getStore('district-vehicle-store');
        var data_time = vehiclesStore.getDataTime();
        if ( typeof data_time == 'undefined' ) {
            return;
        }

        var loadedDate = Ext.Date.parse(data_time, "YmdHi");
        var currDate = new Date();
        var currUtc = new Date(
        ...
        var ageMs = currUtc.getTime() - loadedDate.getTime();
        console.log("Loaded vehicle data has age: " + ageMs + " ms");

        var maxAgeS = emdss.util.Config.getMaxVehiclesAgeS();
        if ( ageMs < (maxAgeS * 1000) ) {
            this.addVehiclesToMap(records, map);
        }
        else {
            console.log("NOT adding vehicles to the map because the are too old: " + ageMs + " ms");
        }
    },
    ...
    createVehiclePopupHtml: function(record) {
        ...
        if ( record.get('temp_f') ) {
            ...
            else html += '<br>Air Temperature: ' + val.toFixed(0) + ' F';
        }
        if ( record.get('humidity') ) {
            var val = parseFloat(record.get('humidity'));
            if ( isNaN(val) || val == -9999.0 ) html += '<br>Humidity: Unknown';
            else html += '<br>Humidity: ' + val.toFixed(0) + ' %';
        }
        if ( record.get('pressure') ) {
            var val = parseFloat(record.get('pressure'));
            var hgVal = val * 0.0295299830714;
            if ( isNaN(val) || val == -9999.0 ) html += '<br>Air Pressure: Unknown';
            else if (emdss.util.Config.getHrefState() == 'alaska') html += '<br>Air Pressure: ' + val.toFixed(0) + ' mb (' + hgVal.toFixed(2) + ' in Hg)';
            else html += '<br>Air Pressure: ' + val.toFixed(0) + ' mb';
        }
        if ( record.get('road_temp_f') ) {
            ...
            else html += '<br>Road Temperature: ' + val.toFixed(0) + ' F';
        }
