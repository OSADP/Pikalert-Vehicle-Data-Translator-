...
Ext.define('emdss.view.site.Show', {
	...
	items : [{
	 ...
	 //    title : 'Weather Cams',
	 //    scrollable : 'both',
	 //    tpl : ['<div><img src="data:image/png;base64,{image}"></img></div>'].join('')
	// }
/*,{
	   },{
	   id:'precip_plots',
...
// 		this.time_series = time_series;
    ...
    updatePlot : function(newRec) {
        console.log("");
        console.log("");
        console.log("");
        console.log("     Got Show::updatePlot()");
        console.log("");
        console.log("");
        console.log("");
// debugger;
    ...
    cachePlot: function(plotToCache) {
        console.log("Caching plot in Show...");
        this.cachedPlot = plotToCache;
    },
    ...
    updateRecord : function(newRec) {
        ...
        console.log("");
        console.log("");
        console.log("     Got Show::updateRecord()");
        console.log("");
        console.log("");
        console.log("");
        ...
        if ( !newRec ) {
            console.log("");
            console.log("         New record is null. Skipping update.");
            console.log("");
        ...
        if ( this.cachedPlot ) {
            newRec.set('summary_plot', this.cachedPlot);
...
// debugger;
		if (newRec && newRec.get('summary_plot') && newRec.get('summary_plot').length > 0) {
		    this.down('#summary_plots').setData(newRec.data);
                    ...
                    // Why was I returning here?
                    // return;
                }

		if (newRec && newRec.time_series() && newRec.time_series().getCount() > 0) {
		    ...
		    if (this.wx_cams) {
				this.remove(this.wx_cams);
				this.wx_cams = null;
		    }
	
			// Get the road time series off the plot data now (used to be from district_alerts)
			this.time_series = newRec.time_series();

			// Various ways to access data on a record...
			// var tester = newRec.data['road_segments'];
			// var testerGet = newRec.get('road_segments');
			// var testerTS = newRec.time_series();
		    ...
		    if (this.time_series) {
				...
				this.setActiveItem(this.alert_plots);
		    }
	
		    // Create the new weather observations tab
	
		    if (newRec.get('wx_obs')) {
				var wx_obs = newRec.get('wx_obs');
			...
			if (newRec.get('road_segments')) {
				var road_segment = newRec.get('road_segments');
				this.road_segment = this.add({
				    ...
				    items : [{
						...
						xtype : 'textfield',
						name : 'model_dew_temp',
						label : 'Model Dewpoint Temp',
						readOnly : true,
						value : road_segment['model_dew_temp']
				    }, {
						xtype : 'textfield',
						name : 'radar_cref',
						...
						xtype : 'textfield',
						name : 'radar_hybrid_reflectivity',
						label : 'Dual Pol Digital Hybrid Reflectivity',
						readOnly : true,
						value : road_segment['radar_hybrid_reflectivity']
				    }, {
						xtype : 'textfield',
						...
						xtype : 'textfield',
						name : 'radar_hydrometeor_classification',
						label : 'Dual Pol Hybrid Hydrometeor Classification',
						readOnly : true,
						value : road_segment['radar_hydrometeor_classification']
				    }, {
						xtype : 'textfield',
						name : 'vehicle_speed_mean',
						label : 'Mean Vehicle Speed',
						readOnly : true,
						value : road_segment['vehicle_speed_mean']
				    }, {
						xtype : 'textfield',
						name : 'num_msg_valid_speed',
						label : 'Number of Valid Speeds',
						readOnly : true,
						value : road_segment['num_msg_valid_speed']
				    }, {
						xtype : 'textfield',
						name : 'vehicle_air_temp_mean',
						label : 'Mean Vehicle Air Temp',
						readOnly : true,
						value : road_segment['vehicle_air_temp_mean']
				    }, {
						xtype : 'textfield',
						name : 'vehicle_bar_pressure_mean',
						label : 'Mean Vehicle Barometric Pressure',
						readOnly : true,
						value : road_segment['vehicle_bar_pressure_mean']
				    }, {
						xtype : 'textfield',
						name : 'vehicle_surface_temp_mean',
						label : 'Mean Vehicle Surface Temp',
						readOnly : true,
						value : road_segment['vehicle_surface_temp_mean']
				    }, {
						xtype : 'textfield',
						name : 'num_wipers_on',
						label : 'Number of Wipers On',
						readOnly : true,
						value : road_segment['num_wipers_on']
				    }, {
						xtype : 'textfield',
						name : 'num_wipers_off',
						label : 'Number of Wipers Off',
						readOnly : true,
						value : road_segment['num_wipers_off']
				    }, {
						xtype : 'textfield',
						name : 'time',
						label : 'Time',
			...
			// Create the new Weather Cams observations tab
			var imageData = newRec.get('image');
			var imageTitle = newRec.get('image_display_text');
			var imageSiteName = this.site_name;
			if ( imageData && imageData.length > 0 ) {
				...
				this.wx_cams = this.add({
				    id : 'wx_cams',
				    title : 'Weather Cams',
				    xtype : 'panel',
				    scrollable : true,
// '    <div>(All alerts generated at: {[this.getPrettyDateText(values.data_time)]})</div>',
				    // tpl : ['<div style="margin-left: 25px; margin-top: 25px;">{[methis.formatIso(values.title)]}<br></div><div style="margin: 25px;"><img src="data:image/png;base64,{image}"></img></div>'].join('')
				    // tpl : ['<div style="margin-left: 25px; margin-top: 25px;">{title}<br></div><div style="margin: 25px;"><img src="data:image/png;base64,{image}"></img></div>'].join('')
				    // tpl : ['<div style="margin: 25px;"><img src="data:image/png;base64,{image}"></img></div>'].join('')
                                    tpl : methis.createWxCamTpl()
				});
				this.wx_cams.setData({isoDateString: imageTitle, siteName: imageSiteName, image: imageData});
	 		}
			else {
				// debugger;
			}
		    /*
		      ...
		      console.log(newRec.get('treatment_explanations'));
		    */
		}
		else {
		    // No plot available...
		    if ( this.down('#summary_plots') ) {
		        this.down('#summary_plots').setHtml('No Summary Available for This Location');
                    }
		}
    },

    createWxCamTpl: function() {
        return new Ext.XTemplate(
            '<div style="margin-left: 25px; margin-top: 25px;">{[this.formatIso(values.isoDateString)]} - {siteName}<br></div>',
            '<div style="margin: 25px;"><img src="data:image/png;base64,{image}"></img></div>',
            {
                formatIso : function(value) {
                    var isoDate = emdss.util.Util.isoStringToDate(value);
                    var prettyDateString = emdss.util.Util.getPrettyDateStringFromDateNoSecs(isoDate);
        ...
        return new Ext.XTemplate(
            ...
            '</div>',
            {
			 ...
			 treatmentAlertCode == 'missing' ) {
                        alert_string =  'Treatment information not available for this site';
                    }
                    else if (treatmentAlertCode == 'clear' ||
			    treatmentAlertCode == 'Clear') {
                        alert_string =  'No treatment recommended at this time';
                    }
