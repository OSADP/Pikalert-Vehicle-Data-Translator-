...
#       All rights reserved.                                                     #
#                                                                                #
#       File: $RCSfile: group_umtri_probe_message_files.py,v $                                           #
#       Version: $Revision: 1.4 $  Dated: $Date: 2014/06/18 20:05:21 $           #
#                                                                                #
# ============================================================================== #
...
# group_umtri_probe_message_files.py -d 2 vii_paths.py /d2/vii/data/static/cdl/probe_message.cdl $VII_DATA/tmp $VII_DATA/processed/mi_csv2nc $VII_DATA/processed/mi_probe_message

# Test
# group_umtri_probe_message_files.py -d 10 vii_paths.py /d2/vii/data/static/cdl/probe_message.cdl /d4/test/umtri/tmp /d4/test/umtri/output /d4/test/umtri/mi_probe_message
#group_umtri_probe_message_files.py -D 20130626 vii_paths.py /d2/vii/data/static/cdl/probe_message.cdl /d4/test/umtri/tmp /d4/test/umtri/output /d4/test/umtri/mi_probe_message
...
def group_umtri_probe_message_files(fname, time_interval, cdl_file, work_dir, out_dir, out_name_schema, logg):
    """
    ...
    if not os.path.exists(work_dir):
        ...
        logg.write_ending(1, "group_umtri_probe_message_files.py")
        exit_value = 1
...
def main():
    ...
    pf = processed_file.ProcessedFile(cf.PROCESSED_LOG_DIR, cf.UMTRI_GROUPED_PROCESSED_BASE)
    ...
    for fname in file_list:
        #if not pf.process_status(fname, date):
        curr_time = time.time()
        file_time = os.path.getmtime(fname)
        if abs(file_time - curr_time) < 600:
            logg.write_info("Writing attempt in processed file for %s" % fname)
            pf.write_processed(date, fname, PROC_ATTEMPT)

            ret_value = group_umtri_probe_message_files(fname, TIME_INTERVAL, cdl_file, work_dir, out_dir, out_name_schema, logg)
            if ret_value != 0:
    ...
    logg.write_ending(0, "group_umtri_probe_message_files.py")
    sys.exit(final_ret)
