...
#       All rights reserved.                                                     #
#                                                                                #
#       File: $RCSfile: create_probe_msg_history.py,v $                                           #
#       Version: $Revision: 1.5 $  Dated: $Date: 2014/06/18 20:34:08 $           #
#                                                                                #
# ============================================================================== #
...
def create_probe_msg_history(cf, in_dir, out_dir, begin_time, end_time, logg):
    ...
    if os.path.exists(output_file):
        ...
        if ret != 0:
            logfile.write_error("Error: The existing output file, %s, could not be removed" % output_file)
            logfile.write_ending(1, "create_probe_msg_history.py")
            return 1
    
    ret = log_msg.run_command(command_string, logg)
    if ret != 0:
        logfile.write_error("ncrcat error")
        logfile.write_ending(1, "create_probe_msg_history.py")
        return 1
