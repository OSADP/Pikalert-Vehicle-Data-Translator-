      ...
      if (bRet == 0)
	{
	  ...
	  for(int hr=0; hr<num_fcst; hr++)
		...
		offset = (hr * metroTimesPerHour) - 1;
	      ...
	      road_cond[current_hr + hr].PsnowOnRoad *= CM_TO_MM; // should this be liquid to snow??

	      //printf("start_hr %d, current_hr %d, hr %d, QP %.10f, TYP %f, SN %f, RC %d, RT %f, SS %f\n", start_hr, current_hr, hr, precipAmt[hr], dpTYP[offset], road_cond[current_hr + hr].PsnowOnRoad, stRC.plArray[offset],  road_cond[current_hr + hr].road_T, road_cond[current_hr + hr].subsurface_T);
	    }
	}
      else
	{
