...
//       Version: $Revision: 1.15 $  Dated: $Date: 2014/03/31 19:50:22 $
//
//==============================================================================
...
public:
  ...
  /** @brief generic vis weight for assessment confidence calculation */
  static const float gen_vis_wgt;
   ...
   */
  int precip_intensity_assessment(vdt_probe_message_qc_statistics& seg_stat, const int precip_type, float &confidence);
...
private:
   ...
   */
  float calc_fog_interest(float rh, float speed_ratio, float station_vis, float percent_fog_lights, float percent_high_beams);
  
  /** @brief calculates generic visibility interest (fuzzy logic)
   * @params[in] relative-humidity, speed-ratio, station-visibility, precentage of lights on, fog-lights on, high-beams on
   * @return visibility interest (0-1)
   */
  float calc_generic_vis_interest(float rh, float speed_ratio, float station_vis, float percent_lights_on, float percent_fog_lights, float percent_high_beams);
