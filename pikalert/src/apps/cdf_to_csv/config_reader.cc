...
//       Version: $Revision: 1.5 $  Dated: $Date: 2013/12/13 22:33:33 $
//
//==============================================================================
...
config_reader::config_reader(const string &input_file_path_arg) : input_file_path(input_file_path_arg)
{
  ...
  try
      ...
      for (int i=0; i<input_output_field_setting.getLength(); i++)
	{
      ...
      // get time string map
      Setting &time_string_formats_setting = cfg.lookup("RWX.input.time_string_formats");
      for (int i=0; i<time_string_formats_setting.getLength(); i++)
	{
	  Setting &pair_setting = time_string_formats_setting[i];
	  string input_field_name = pair_setting[0];
	  string time_string_format = pair_setting[1];
	  if (input_type_index_map.find(input_field_name) == input_type_index_map.end())
	    {
	      error = string("input field name in time_string_formats: ") + input_field_name + string(", ") + time_string_format + string(" cannot be found in the field name lists");
	      return;
	    }
	  time_string_format_map[input_field_name] = time_string_format;
	}
    }
