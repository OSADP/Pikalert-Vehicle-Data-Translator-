...
int process::run()
{
  ...
  for (auto rwh_itr=rwh_record_map.begin(); rwh_itr!=rwh_record_map.end(); ++rwh_itr)
	...
	{
	  Logg->write_time_error("could not find site number %d from rwh file %s in road_segment_map obtained from %s\n", road_site_num, args.rwh_input_file.c_str(), cfg_reader.road_segment_file.c_str());
	  return -1;
	}
