...
private:
  ...
  void build_range_tests(const std::string& test_key, vector<QC_CHECK_PTR>& tests)const;
  void build_climate_tests(vector<QC_CHECK_PTR>& tests)const;
  void build_persistence_tests(time_t begin_time, time_t end_time, vector<HISTORY_CHECK_PTR>& history_check)const;
  void build_step_tests(time_t begin_time, time_t end_time, vector<HISTORY_CHECK_PTR>& history_check)const;
};
