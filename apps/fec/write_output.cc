		 ...
		 long *num_output_days, long *num_output_times_per_day)
{
  ...
  // Get sites dimension, handle unlimited size
		      ...
		      long *num_output_times_per_day, long *output_max_str_len, long *output_total_num_times)
{
  ...
  // Write out type
  out_var_ptr = output_file->get_var("type");
  if (!out_var_ptr)
    {
      proc_log->write_time("Error: Unable to get ptr for %s\n", "type");
      return(1);
  ...
  ret = out_var_ptr->put(&output_type, 1);
  if (ret == 0)
    {
      proc_log->write_time("Error: Unable to put data for var %s\n", out_var_ptr->name());
      return(1);
    ...
    proc_log->write_time(1,"Info: type is %d\n", output_type);
  ...
  *max_output_sites = max_sites;
  ...
  NcDim *max_str_len_dim = output_file->get_dim("max_str_len");
  *output_max_str_len = max_str_len_dim->size();

  NcDim *total_num_times_dim = output_file->get_dim("num_times");
  *output_total_num_times = total_num_times_dim->size();
