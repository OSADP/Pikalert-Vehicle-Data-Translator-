			...
			long *total_num_times)
{
  ...
  *forc_time = ft;
  ...
  // Get type variable
  //
  input_var_ptr = input_file->get_var("type");
  if (!input_var_ptr)
    {
      proc_log->write_time("Error: Unable to get ptr for type\n");
      return(1);
    }

  ret = input_var_ptr->get(type, 1);
  if (ret == 0)
    {
      proc_log->write_time("Error: Unable to get data for var %s\n",
		       input_var_ptr->name());
  ...
  *num_input_times_per_day = num_input_times_dim->size();
  ...
  NcDim *max_str_len_dim = input_file->get_dim("max_str_len");
  *max_str_len = max_str_len_dim->size();

  NcDim *total_num_times_dim = input_file->get_dim("num_times");
  *total_num_times = total_num_times_dim->size();
		   ...
		   long *times_per_day, char *data)
{
  ...
  *times_per_day = in_dim_array[2];
  ...
  // allocate space for raw data if necessary
  //cout << "num_dims = " << num_dims << " raw_data_size = " << raw_data_size << endl;
  ...
  // Just copy the char data from input array (raw_data) to output array (data)
  delete [] in_dim_array;
