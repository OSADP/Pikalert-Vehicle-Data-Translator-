...
int ChemConc_InitAndGetParams( int ctype, int cform, SiteLayerTraffic *site_config, int ptflag, int app_rate_units, ChemicalType *c )
{
  ...
  // (Not sure why 5 degrees are subtracted from this - why not just use the halfpoint temp? Cowie)
  P_ROADT_TOOCOLD = (int)(c->MinTemp - 5.0);

  // Set the in-range value between 0 and the too cold value
  P_ROADT_INRANGE = (int)(P_ROADT_TOOCOLD/2);
