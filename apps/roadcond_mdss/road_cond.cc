	      ...
	      char *site_config_name, char *output_name)
{
  ...
  for (ns = 0; ns < num_output_sites; ns++)
	...
	InitNewTreatment( &TreatmentList[i] );
      ...
      // temperature profile (Moved below the chem type initialization so they
      // are not missing - Cowie)
      //
	...
	case SUGGESTED_TREATMENT:
	  // changed to use the suggested treatment type here so snow will accumulate on the
	  // road. This accompanies a change in the metro code which allows snow to accumulate
	  // for rec & no tmt. If this is not done, we don't get a realistic storm characterization
	  // and the treatments become diminished. At some point, this code block can be collapsed
	  // with the case above since they all now use the actual treatment. (Cowie)
	  ret = calc_road_temp(99, ns, output_latlon_array[ns*2], output_latlon_array[ns*2+1],
			       output_site_array[ns], forc_time, roadT_current_hour, chem_failure_time,
			       //SUFFICIENT_TREATMENT,
			       SUGGESTED_TREATMENT,
			       metro_run_type, use_flux, num_fcst, nt_start,
			       bridge[ns], obs_start_time,
			       &wx_fcst_ts[ns], &curr_obs, &site_layer[ns], roadcond);
	  if (ret > 1)
	    {
