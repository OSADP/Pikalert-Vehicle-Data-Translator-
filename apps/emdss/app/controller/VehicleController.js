...
Ext.define('emdss.controller.VehicleController',{
    ...
    init: function() {
		...
		this.districtVehicleStore.setProxy({
		    type:'ajax',
		    url: emdss.util.Config.getVehiclesUrl() + '&state=' + emdss.util.Config.getHrefState(),
    ...
    onVehicleMapClick: function(e, mark, lt, rec, map) {
		...
		me.vehicleOverlay = Ext.getCmp('map-nav-view').add({
		    xtype: 'panel',
	//			xtype: 'container',
		    layout: 'fit',
		    modal: true,
    ...
    createVehiclePopupHtml: function(record) {
	    ...
	    toggleShowVehicles: function(button) {
		var me = this;
