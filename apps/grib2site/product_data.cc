 ...
 */
/* $Id: product_data.cc,v 1.18 2013/11/07 18:34:50 dicast Exp $ */
		 ...
		 product_data *out)
{
    ...
    if (out->center == CENTER_ECMWF){
      out->param = param_code_e(out->edition, g1i(pdsp->param)) ;
		 ...
		 product_data *out)
{
  ...
  if (param_g21(g2fld->ipdtnum, g2fld->discipline, g2fld->ipdtmpl[0],
		g2fld->ipdtmpl[1], &out->param_vers, &out->param) != 0) {
    return -5;
  }

  // Map grib2 levels to grib1.
  if (level_g21(g2fld->ipdtmpl, &out->level_flg, out->level) != 0) {
    return -4;
  }
  ...
  if (g2fld->ipdtnum <= 7) {
    ...
    out->tr[1] = out->tr[0] + g2fld->ipdtmpl[offset+3];
...
/* 
 * Modify product data structure for certain ECMWF grids.
 */
void modify_ecmwf_pdata(product_data *out, pds *pdsp)
{
  ...
  if (out->level_flg == LEVEL_SURFACE){
    ...
    /* Change level type and value for "100 metre U wind component"  */
    if (pdsp->param == 246){
      out->level_flg = LEVEL_FHG;
      out->level[1] = 100;
    }
	
    /* Change level type and value for "100 metre V wind component"  */
    if (pdsp->param == 247){
      out->level_flg = LEVEL_FHG;
      out->level[1] = 100;
    }
...
/* 
 * Modify product data structure for certain BOM grids.
 */
