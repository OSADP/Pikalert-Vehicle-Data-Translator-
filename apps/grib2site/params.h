 ...
 */
/* "$Id: params.h,v 1.10 2014/11/25 23:26:05 dicast Exp $" */
...
#define PARM_ICAHT	5	/* ICAO Standard Atmosphere Reference Height, m*/
...
#define PARM_TOZNE	10	/* Total Ozone, Dobsons */
...
#define PARM_PLI	24	/* Parcel lifted index (to 500 hPa), K*/
...
#define PARM_BLI	77	/* Best lifted index (to 500 hPa), K*/
...
#define PARM_BRTMP	118	/* Brightness temperature, K*/
#define PARM_LWRAD	119	/* Long-wave radiation, W/srm2*/
#define PARM_SWRAD	120	/* Short-wave radiation, W/srm2*/
...
#define PARM_SOILW	144	/* Volumetric soil moisture content, fraction */
#define PARM_PEVPR	145	/* Potential eveopration rate, W/m2 */
#define PARM_CWORK	146	/* Cloud workfunction, J/kg  */
#define PARM_U_GWD	147	/* Zonal flux of gravity wave stress, N/m2  */
#define PARM_V_GWD	148	/* Meridional flux of gravity wave stress, N/m2  */
#define PARM_PVORT      149	/* Potential vorticity, m2/s/kg */
...
#define PARM_CLWMR	153	/* Cloud water, kg/kg*/
#define PARM_O3MR	154	/* Ozone mixing ratio, kg/kg*/
...
#define PARM_RWMR	170	/* Rain water mixing ratio kg/kg */
#define PARM_SNMR	171	/* Snow mixing ratio kg/kg */
...
#define PARM_ICMR	178	/* Ice mixing ratio kg/kg */
#define PARM_GRMR	179	/* Graupel mixing ratio kg/kg */

#define PARM_GUST	180	/* Wind gust in m/s */
...
#define PARM_VPTMP	189	/* Virtual potential temperature, K*/
#define PARM_HLCY	190	/* Storm relative helicity, m2/s2*/
#define PARM_PROB	191	/* Probability from ensemble, percent*/
#define PARM_PROBN	192	/* Probability from ensemble normalized with respect to climate expectancy, percent*/
#define PARM_POP	193	/* Probability of precipitation, percent*/
#define PARM_CPOFP	194	/* Probability of frozen precipitation, percent*/
#define PARM_CPOZP	195	/* Probability of freezing precipitation, percent*/
...
#define PARM_HPBL	221	/* Planetary boundary layer height, m*/
...
#define PARM_GPT_HGT5A	230	/* 5-wave geopotential height anomoly, gpm */
...
#define PARM_03TOT	237	/* Total ozone, Kg/m2*/
...
// These are variables from the ECMWF which have different units than above,
// so we have to enumerate them separately.

#define PARM_ECMWF_PRECIP_TOT  278 /* Total precipitation, meters */
#define PARM_ECMWF_PRECIP_CN   279 /* Convective precipitation, meters */
#define PARM_ECMWF_DSWRF       280 /* downward short wave rad. flux, W/m2-s */

// Added these for BOM ACCESS grids, since there does not seem to be a close
// enough match in the existing list (Cowie)

#define PARM_SEA_ICE     281    /* sea ice cover 0-1 */
#define PARM_AREA_CLOUDF 282    /* cloud area fraction 0-1 */
#define PARM_LIQ_CLOUDF  283    /* liquid cloud fraction 0-1 */
#define PARM_ICE_CLOUDF  284    /* ice cloud fraction 0-1 */
#define PARM_EVAP_SEA    285    /* evaporation over open sea in kg/m2 */
#define PARM_FOG_FRACT   286    /* fog fraction 0-1 */
#define PARM_VIS_PROB    287    /* probability of horizontal visibility < 5km 0-1 */
#define PARM_PW_CODE     288    /* present weather code (unitless, UKMET) */
#define PARM_CSRAT       289    /* Convective snowfall rate, kg/m2/s */

// Added this for Composite Reflectivity (to get from NCEP HRRR and other models)
//
#define PARM_REFC        290    /* Composite reflectivity [dB]:entire atmosphere */

#define PARM_LAST_ENTRY  291	/* Keep this current if table is expanded.
...
extern "C" int param_code_e ( int grib_edition, int param );
extern "C" int param_code_ukmet ( int param );
extern "C" int param_code_bom ( int grib_table_version, int param );
extern "C" int sfcparam(int param);
extern "C" int mslparam(int param);
extern "C" int lisoparam(int param);
extern "C" int param_g21(GRIB2::g2int g2pdtnum, GRIB2::g2int g2mdis,
			 GRIB2::g2int g2pcat,
...
extern int param_code_e ( int grib_edition, int param );
extern int param_code_ukmet ( int param );
extern int param_code_bom ( int grib_table_version, int param );
extern int sfcparam(int param);
extern int mslparam(int param);
extern int lisoparam(int param);
extern int param_g21(GRIB2::g2int g2pdtnum, GRIB2::g2int g2mdis,
		     GRIB2::g2int g2pcat,
		     GRIB2::g2int g2pnum, int *g1pver, int *g1pnum);
#else
extern char* grib_pname (/* int param_code */);
...
extern int param_g21(/* GRIB2::g2int g2pdtnum, GRIB2::g2int g2mdis,
			GRIB2::g2int g2pcat,
			GRIB::g2int g2pnum, int *g1pver, int *g1pnum */ );
#endif
