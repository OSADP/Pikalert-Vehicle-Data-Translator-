...
int make_site_data(product_data *pd, float fillval, char *calc_type, float *lat_arr, float *lon_arr, int num_sites, float *site_data)
{
  ...
  // stereographic grids. We use the dmapf library to do the lat/lon <->
  // grid coord translation. Note that lat/lon grid does not actually use
  // the dmapf translations, we do it ourselves.
  ...
  for (ns=0; ns<num_sites; ns++)
    {
	  ...
	  // if la1 < la2, grid is oriented south to north, if la1 > la2,
	  // it is oriented north to south
	  if (la1 < la2)
	    y = (lat - la1)/dely + jref;
	  else
	    y = (la1 - lat)/dely + jref;
      ...
      // Check that the location is on the grid. The special check on the x
      // coordinate is to permit lat-lon grids that wrap globally.
      if ((y < 0.) || (y > ny-1) || (x < 0.) || (x > nx) ||
	  (pd->gd->type != GRID_LL && nx*delx != 360 && x > nx-1))
        {
	  logFile->write_time(3, "Info: lat %f, lon %f, x %f, y %f, value (off grid)\n", lat_arr[ns], lon_arr[ns], x, y);
          continue;
        }
      ...
      else
	{
      ...
      logFile->write_time(3, "Info: lat %7.2f, lon %7.2f, x %.2f, y %.2f, value %f\n", lat_arr[ns], lon_arr[ns], x, y, site_data[ns]);
