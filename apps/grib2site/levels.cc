 ...
 */
/* $Id: levels.cc,v 1.7 2014/11/25 21:39:31 dicast Exp $ */
...
int level_g21(GRIB2::g2int *ipdtmpl, int *level_flg, int *levels)
{
  int ltype1, ltype2;
  float scale1, scale2;
  ...
  *level_flg = 255;
  ...
  if (ltype1 == 10 && ltype2 == 255) {             // Special case for NCEP HRRR to get total-cloud-cover (Surface to top of Atm). LEVEL_ATM = 200
    ...
    *level_flg = 255;
    levels[0] = 0;
    levels[1] = 0;
    logFile->write_time(1, "Error: Can't process GRIB2 level type(s): %d, %d, scale(s) %f, %f\n", ltype1, ltype2, scale1, scale2);
    return (-1);
  }
  ...
  if (levels[0] > 255 || levels[1] > 255) {
    logFile->write_time(1, "Error: Processing GRIB-2 level type %d, %d\n", ltype1, ltype2);
    return (-1);
  }
