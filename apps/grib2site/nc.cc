 ...
 */
/* $Id: nc.cc,v 1.31 2013/11/22 19:19:51 dicast Exp $ */
	...
	)
{
    ...
    //
    // Special case for accumulated quantities: Add the accumulation interval
    // if the base variable name does not exist in the output file.
    //
    if (tr_flg == TRI_Acc) {
	...
	// We used to discriminate by center ID ... 
	//
	// if (pp->center != CENTER_CMS && pp->center != CENTER_BOM &&
	//    pp->center != CENTER_NL)
	...
	//
	// If not found, try name with time range
	//
	if (ret != NC_NOERR)
	  {
	    switch (pp->tunit) {
