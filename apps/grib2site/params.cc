 ...
 */
/* "$Id: params.cc,v 1.20 2014/12/04 16:24:27 dicast Exp $" */
...
struct param_table ptable[] = {
    ...
    {PARM_PREIX,	(char *)"preix",	(char *)"fraction"},
...
/*
 * Maps ECMWF parameter codes to unique parameter code. Needs grib edition
 * but not parameter version table.
 */
int
	...
	)
{
    switch (grib_edition) {
      case 1:
	switch (param) {
	  case  28: return PARM_GUST;
	    //	  case 123: return PARM_GUST;
	  case 129: return PARM_GPT_HGT;
 ...
 */
int param_g21(GRIB2::g2int g2pdtnum, GRIB2::g2int g2mdis, GRIB2::g2int g2pcat,
	      GRIB2::g2int g2pnum, int *g1pver, int *g1pnum)
{
  ...
  for (int i=0; i<num_gparams; i++) {
    ...
    if (gparams[i].g2mdis == g2mdis && gparams[i].g2pcat == g2pcat &&
      ...
      *g1pver = gparams[i].g1pver;
      //
      // Check for special cases of probability forecasts. Will need to
      // generalize this someday for other variables. Also for percentile
      // forecasts.
      //
      if (g2pdtnum == 5 || g2pdtnum == 9) {

	// Visibility
	if (*g1pnum == 20)
	  *g1pnum = 287;
	else
	  logFile->write_time(1, "Warning: GRIB2 PDT number indicates variable is probabilistic, but no translation is available.\n");
  ...
  // Didn't find
  logFile->write_time(1, "Info: Unknown GRIB-2 parameter: discipline %d, category %d, number %d\n", g2mdis, g2pcat, g2pnum);
