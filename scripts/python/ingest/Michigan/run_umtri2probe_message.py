...
"""Convert umtri csv files to netcdf probe message format"""
...
#       All rights reserved.                                                     #
#                                                                                #
#       File: $RCSfile: run_umtri2probe_message.py,v $                                           #
#       Version: $Revision: 1.3 $  Dated: $Date: 2014/08/20 19:02:36 $           #
#                                                                                #
# ============================================================================== #
...
# run_umtri2probe_message.py -l /d2/vii/data/log/run_umtri2probe_message vii_paths.py 20130625 /d2/vii/data/raw/umtri/ /d2/vii/data/processed/mi_csv2nc /d2/vii/data/static/cdl/probe_message.cdl
...
def run_umtri2probe_message(cf, date, input_dir, out_dir, cdl, logg):
    """
    ...
    pf = processed_file.ProcessedFile(cf.PROCESSED_LOG_DIR, cf.UMTRI2NC_PROCESSED_BASE)
    final_ret = 0
    ...
    if not os.path.exists(dated_output_dir):
        ...
        if ret != 0:
            ...
            logg.write_ending(1, "run_umtri2probe_message.py")
            return 1    
    ...
    for fname in file_list:
        ...
        if reprocess != ALWAYS_REPROCESS and pf.process_status(fname, date) != (processed_file.PROC_NONE, 0):
            ...
            #print "%s has already been processed, skipping" % fname
            #logg.write_info("%s has already been processed, skipping" % fname)
            already_processed_ct += 1
            continue

        logg.write_info("Writing attempt in processed file for %s" % fname)
        ...
        log_file = cf.LOG_DIR + "/" + "umtri2probe_message"

        umtri_ret = 0
        
        #cmd = "%s umtri2probe_message.py -l %s %s %s %s %d" % (cf.ENTHOUGHT_PYTHON, log_file, cdl, fname, output_file_path, umtri_ret)
        cmd = "umtri2probe_message.py -l %s %s %s %s %d" % (log_file, cdl, fname, output_file_path, umtri_ret)
        ret = log_msg.run_command(cmd, logg)
        
        if umtri_ret != 0:
            logg.write_error("error calling umtri2probe_message.py")
            logg.write_ending(1, "run_umtri2probe_message.py")
            return 1
        else:
            logg.write_info("Writing success in processed file for %s" % fname)
            pf.write_processed(date, fname, PROC_SUCCESS)

    logg.write_ending(final_ret, "run_umtri2probe_message.py")
    sys.exit(final_ret)
...
def main():
    ...
    if options.log:
        ...
        logg = log_msg.LogMessage("run_umtri2probe_message.py")
    ...
    logg.write_info("running on: %s %s %s %s %s" % (config_file, date, in_dir, out_dir, cdl))
    run_umtri2probe_message(cf, date, in_dir, out_dir, cdl, logg)
