...
def add_can(ls, logg, data_dict):
    ...
    active_can_spn_tokens = set([108, 171])
    for spn in spn_list:
        ls = spn.split(" ")
        if len(ls) < 3:
            continue
        (num_string, val_string, unit_string) = ls
        try:
            num = float(num_string)
            if num in active_can_spn_tokens:
                val = float(val_string)
            else:
                continue
        except ValueError:
            logg.write_error("missing floating point value in can spn_list for bar_pressure and air_temp: num_string %s, val_string %s, original spn_list_string %s" % (num_string, val_string, spn_list_string))
            continue
