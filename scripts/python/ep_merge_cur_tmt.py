...
# Runs merge to combine rc_cur_tmt and bc_cur_tmt
...
Road_cond_cur_tmt_dir = "%s/%s" %  (rctm_path.Rctm_root_dir, "fec_rc_cur_tmt")
Bridge_cond_cur_tmt_dir = "%s/%s" %  (rctm_path.Rctm_root_dir, "bc_cur_tmt")

input_dirs = "%s %s" % (Road_cond_cur_tmt_dir, Bridge_cond_cur_tmt_dir)

age = "86400 86400"

output_dir = "%s/%s" %  (rctm_path.Rctm_root_dir, "merge_cur_tmt")

log_base = "%s/%s" % (rctm_path.Log_dir, "merge_cur_tmt")

params_file = "%s/%s" % (Params_dir, "merge_cur_tmt.params")
