    ...
    def extract_temp_dewp(self):
        ...
        if self.temps == None and self.dewps == None and self.temp_units == None and self.dewp_units == None:
            if self.rdwx_site_index != ():
                ...
                #print "dewps"
                #print len(dewps), dewps
                #print "temps >="
                temps[temps>=9.9592099683868690e+36] = nan
                dewps[dewps>=9.9592099683868690e+36] = nan

                #print "current hour: ", self.current_utc_hour  
...
def make_alert_bar(ax_list, time_alerts, rdwx_nc_object, all_sites, site_num):
    ...
    # Note that labelpad is used to pad the label from the left side of the plot
    alert_bar_ax.set_ylabel("Alert\n Summary", fontsize=8, fontweight='bold', rotation=0, labelpad=32)
    if not rdwx_nc_object.rdwx_error and len(dates) > 0:
...
def make_rec_treatments_bar(ax_list, time_tmts, time_alerts, tmt_nc_object, snow_accum_plot_bool):
    ...
    # Note that labelpad is used to pad the label from the left side of the plot
    rec_tmt_ax.set_ylabel('Recommended\n Treatments', fontsize=8, fontweight='bold', rotation=0, labelpad=35)
