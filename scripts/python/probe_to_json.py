...
def get_full_input_dir(cf, yyyymmdd, state):
    return {"mn":os.path.join(cf.Minnesota_vdt_output_dir, yyyymmdd),
            "mi": os.path.join(cf.Michigan_vdt_output_dir, yyyymmdd),
            "nv": os.path.join(cf.Nevada_vdt_output_dir, yyyymmdd),
            "co": os.path.join(cf.Colorado_vdt_output_dir, yyyymmdd)}[state]

def get_parent_output_dir(cf, state):
    return {"mn":cf.Minnesota_latest_vehicles_dir,
            "mi":cf.Michigan_latest_vehicles_dir,
            "nv":cf.Nevada_latest_vehicles_dir,
            "co":cf.Colorado_latest_vehicles_dir}[state]

def get_full_output_dir(cf, yyyymmdd, state):
    return {"mn":os.path.join(cf.Minnesota_latest_vehicles_dir, yyyymmdd),
            "mi":os.path.join(cf.Michigan_latest_vehicles_dir, yyyymmdd),
            "nv":os.path.join(cf.Nevada_latest_vehicles_dir, yyyymmdd),
            "co":os.path.join(cf.Colorado_latest_vehicles_dir, yyyymmdd)}[state]
...
def run_probe_to_json(yyyymmdd, hhmm, state, cf, logg):
    ...
    full_input_dir = get_full_input_dir(cf, yyyymmdd, state)
    input_path = os.path.join(full_input_dir, nc_fname)
    
    parent_output_dir = get_parent_output_dir(cf,state)
    full_output_dir = get_full_output_dir(cf, yyyymmdd, state)
...
def probe_to_json(state, cf, interval_time_sec, logg, options):
    ...
    if not options.begin_date and not options.end_date:    
        ...
        for mydate in date_list:
            full_input_dir = get_full_input_dir(cf, mydate, state)
