...
class Log_error(Exception):
     def __init__(self,value):
         self.value = "log.py module error: %s" % value
     def __str__(self):
...
class Log:
	...
	if self.log_path != "":
		...
		try:
                    ...
                    print lfp
		    raise Log_error, "write: file error on %s, %s, %s" % (lfp, sys.exc_type, sys.exc_value)
	else:
	    self.fp = sys.stdout
	...
	try:
	    ...
	    raise Log_error, "write: file processing error"
...
if __name__ == '__main__':
    log = Log("log_test")
    log.set_suffix(".glu")
    log.write("rolling the logs\n")
    log.write("rolling the logs again\n")
    log.write_time("rolling the logs\n")
    log.write_time("rolling the logs again\n")
