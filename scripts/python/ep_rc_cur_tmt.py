...
# Runs roadcond_mdss with the current-treatment option.
...
site_config_file = "%s/%s" % (Config_dir, "site_config.conf")
treatment_option = "2"
...
Road_cond_init_dir  = "%s/%s" %  (rctm_path.Rctm_root_dir, "rc_init")
Rdwx_fcst_dir = "%s/%s" %  (rctm_path.Rctm_root_dir, "rdwx_fcst")
Obs_hist_dir = "%s/%s" %  (rctm_path.Rctm_root_dir, "obs_hist")

input_dirs = "%s %s %s %s" % (Current_plan_dir, Road_cond_init_dir, Obs_hist_dir, Rdwx_fcst_dir)

age = "86400 86400 86400 86400"

output_dir = "%s/%s" %  (rctm_path.Rctm_root_dir, "rc_cur_tmt")

log_base = "%s/%s" % (rctm_path.Log_dir, "rc_cur_tmt")
...
plan_root_dir = rctm_path.Rctm_root_dir

# Make default plans as needed
ret = plan.mk_def_plan(plan_root_dir, site_list)

# Create concatenated current plan file
ret = plan.mk_current_plan(plan_root_dir, fcst_time)

use_def_mat_str = "-mat_default ASPHALT"
