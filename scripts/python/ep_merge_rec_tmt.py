...
# Runs merge to combine rc_rec_tmt and bc_rec_tmt
...
Road_cond_rec_tmt_dir = "%s/%s" %  (rctm_path.Rctm_root_dir, "fec_rc_rec_tmt")
Bridge_cond_rec_tmt_dir = "%s/%s" %  (rctm_path.Rctm_root_dir, "bc_rec_tmt")

input_dirs = "%s %s" % (Road_cond_rec_tmt_dir, Bridge_cond_rec_tmt_dir)

age = "86400 86400"

output_dir = "%s/%s" %  (rctm_path.Rctm_root_dir, "merge_rec_tmt")

log_base = "%s/%s" % (rctm_path.Log_dir, "merge_rec_tmt")

params_file = "%s/%s" % (Params_dir, "merge_rec_tmt.params")
